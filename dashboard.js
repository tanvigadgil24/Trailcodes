...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document.getElementById("submitTreeButton").addEventListener("click", function () {
    if (userIsAdmin()) {
      submitTreeData();
    } else {
      voteTree();
...
function userIsAdmin() {
  var isAdmin = false; // Assume user is not admin by default
...
function voteTree() {
  ...
  var treeData = {
    tree_id: generateTreeId(), // Generate a unique tree id
    latitude: lat,
    longitude: lng,
  ...
  fetch("http://localhost:5000/vote_tree", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
    ...
    .then((data) => {
      ...
      if (data.status === "success") {
        ...
        alert("Failed to add vote: " + data.message);
      }
    })
    .catch((error) => {
