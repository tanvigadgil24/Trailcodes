...
# new code for tree addtion
@app.route("/vote_tree", methods=["POST"])
@login_required
def vote_tree():
    data = request.json
    ...
    try:
        # tree_id = int(data["tree_id"])  # Convert tree_id to integer
        tree_type = data["tree_type"]  # Get tree type from request
        height = float(data["height"])  # Convert height to float
        age = int(data["age"])  # Convert age to integer
        latitude = float(data["latitude"])  # Convert latitude to float
        longitude = float(data["longitude"])  # Convert longitude to float
    ...
    try:
        ...
        if current_user.is_admin:  # Replace this with your admin check logic
            # Insert the tree directly into the database if user is admin
            cur.execute(
                "INSERT INTO tanvitree (type, height, age, latitude, longitude) VALUES (%s, %s, %s, %s, %s)",
                (type,height,age,latitude,longitude),
            )
            conn.commit()
            cur.close()
            conn.close()
        ...
        # If user is not admin, proceed with the voting logic
        # Insert the user's vote into the tree_votes table
        else:
             cur.execute(
            "INSERT INTO tanvitree_votes (tree_type, height, age, latitude, longitude, user_id) VALUES (%s, %s, %s, %s, %s, %s)",
            (tree_type, height, age, latitude, longitude,current_user.id),
        )
        conn.commit()
        message = "Tree vote added successfully."

        # Fetch all votes for this geolocation
        cur.execute(
            "SELECT tree_type FROM tanvitree_votes WHERE latitude = %s AND longitude = %s",
            (latitude, longitude),
        )
        all_votes = cur.fetchall()

        # Count the occurrences of each tree type
        tree_type_counts = Counter(vote[0] for vote in all_votes)

        # Find the tree type with the maximum count
        max_tree_type = max(tree_type_counts, key=tree_type_counts.get)

        # Fetch all votes for the tree type with maximum count
        cur.execute(
            "SELECT * FROM tanvitree_votes WHERE latitude = %s AND longitude = %s AND tree_type = %s",
            (latitude, longitude, max_tree_type),
        )
        max_votes = cur.fetchall()

        # Select one of the votes randomly if there are multiple votes for the maximum tree type
        selected_vote = random.choice(max_votes)

        # Insert the selected vote into the tree_requests table
        cur.execute(
            "INSERT INTO tree_requests (latitude, longitude, tree_type, height, age, user_id) VALUES (%s, %s, %s, %s, %s, %s, %s)",
            (latitude, longitude, selected_vote[3], height, age, current_user.id),
        )
        conn.commit()
        ...
        return jsonify({"status": "success", "message": "Vote added successfully"}), 201

    except Exception as e:
        return (
            jsonify({"status": "fail", "message": "Failed to add the vote", "error": str(e)}),
            500,
        )
